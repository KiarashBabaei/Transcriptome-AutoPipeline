...
# === SRA sample IDs (selected only) ===
srr_list_file = os.path.join(base_dir, "selected_SRR_ids.txt")

if not os.path.exists(srr_list_file):
    sys.exit("ERROR: selected_SRR_ids.txt not found. Please run merge_geo_sra.py first.")

with open(srr_list_file) as f:
    ids = [line.strip() for line in f if line.strip()]

if not ids:
    sys.exit("ERROR: No SRR IDs found in selected_SRR_ids.txt.")

print(f"Loaded {len(ids)} SRR IDs from selected_SRR_ids.txt")
